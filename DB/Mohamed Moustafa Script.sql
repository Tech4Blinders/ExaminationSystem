	...
	if exists(select Std_id from Student where Std_id = @std_id)
		begin
			if exists(select crs_id from Course where crs_id = @course_id)
				begin
					exec SP_Insert_Exam @exam_type, @no_mcq, @no_tf
					declare @exam_id as int
					select @exam_id = max(ex_id) from Exam;
					declare @quest_id as int
					declare @i int = 0;
					while @i < @no_mcq
						begin
							select @quest_id = (select top 100 min(q_id) from Question where q_score = 2 and crs_id = @course_id ORDER BY NEWID());
					...
					while @i < @no_tf
						begin
							select @quest_id = (select top 100 min(q_id) from Question where q_score = 1 and crs_id = @course_id ORDER BY NEWID());
							exec SP_Insert_StdQuesAnswer @std_id, @exam_id, @quest_id, null;
							set @i = @i + 1;
						end
				end
...
Making_Exam 1, 1, 10, 5, 'Quiz1'

select top 100 q_id from Question where q_score = 2 and crs_id = 1 ORDER BY NEWID()
